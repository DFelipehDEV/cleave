...
void Shader::CreateFromFile(const std::string& vertexPath,
    ...
    if (fragmentSource.empty()) {
        std::cerr << "ERROR::SHADER::FRAGMENT_SHADER_LOAD_FAILED ("
                  << fragmentPath << ")\n";
        return;
    }
...
void Shader::SetUniformVector2f(const std::string& name, float x,
                                float y) const {
    GLint location = glGetUniformLocation(m_id, name.c_str());
    if (location == -1) {
        std::cerr << "ERROR::SHADER::UNIFORM_NOT_FOUND (" << name << ")\n";
...
void Shader::SetUniformVector3f(const std::string& name, float x, float y,
                                float z) const {
    GLint location = glGetUniformLocation(m_id, name.c_str());
    if (location == -1) {
        std::cerr << "ERROR::SHADER::UNIFORM_NOT_FOUND (" << name << ")\n";
...
void Shader::SetUniformVector4f(const std::string& name, float x, float y,
...
void Shader::SetUniformMatrix4(const std::string& name,
